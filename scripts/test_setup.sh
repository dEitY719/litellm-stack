...
if echo "$MODELS" | grep -q "gemini-2.5-pro\|gemini-pro"; then
    echo -e "${GREEN}✓ Gemini 모델 등록됨${NC}"
else
    echo -e "${YELLOW}⚠ Gemini 모델을 찾을 수 없습니다${NC}"
    echo "  확인: docker compose logs litellm | grep -i gemini"
    echo "  설정: .env 파일에서 GEMINI_API_KEY 확인"
fi
...
if echo "$MODELS" | grep -q "tinyllama"; then
    ...
    echo "  해결: docker exec ollama ollama pull tinyllama"
    exit 1
fi
...
RESPONSE=$(curl -s -X POST http://localhost:4444/v1/chat/completions \
    ...
    -d '{"model": "tinyllama", "messages": [{"role": "user", "content": "Hello"}], "max_tokens": 10}' 2>/dev/null)
...
echo -e "  ${BLUE}python example/test_langchain_agent.py${NC}\n"
